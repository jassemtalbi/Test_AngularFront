import { Component, OnInit } from '@angular/core';
import { NgForm } from '@angular/forms';
import { Router } from '@angular/router';
import { TestService } from '../services/test.service';

@Component({
  selector: 'app-add-new-employe',
  templateUrl: './add-new-employe.component.html',
  styleUrls: ['./add-new-employe.component.css']
})
export class AddNewEmployeComponent implements OnInit {
  constructor(private TestService: TestService,private route:Router) { }
  ngOnInit(): void {
  }
  AddEmploye(form: NgForm) {
    const value = form.value;

    this.TestService.AddEmploye(value.userName,value.email,value.password,value.rpassword,value.id_Department,value.id_role).subscribe(
      (data) => {
      
          console.log(data);
          this.route.navigate(['/User']); // navigate to other page
          
      },
      errors => {
        console.log(errors);
   
      },
    )
    ;
  }
}
